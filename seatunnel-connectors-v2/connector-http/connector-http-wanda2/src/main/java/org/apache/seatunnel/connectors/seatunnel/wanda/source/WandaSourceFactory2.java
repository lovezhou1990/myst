...
public class WandaSourceFactory2 extends HttpSourceFactory {
    @Override
    ...
    TableSource<T, SplitT, StateT> createSource(TableSourceFactoryContext context) {
        return () ->
                (SeaTunnelSource<T, SplitT, StateT>)
                        new WandaSource2(context.getOptions().toConfig());
    }

    @Override
    public OptionRule optionRule() {
        return getHttpBuilder()
                .required(WandaSourceConfig2.APP_KEY)
                .required(WandaSourceConfig2.APP_SECRET)
                .build();
    }
}
