...
public class WandaSourceFactory extends HttpSourceFactory {
    @Override
    ...
    TableSource<T, SplitT, StateT> createSource(TableSourceFactoryContext context) {
        return () ->
                (SeaTunnelSource<T, SplitT, StateT>)
                        new WandaSource(context.getOptions().toConfig());
    }

    @Override
    public OptionRule optionRule() {
        return getHttpBuilder()
                .required(WandaSourceConfig.APP_KEY)
                .required(WandaSourceConfig.APP_SECRET)
                .build();
    }
}
